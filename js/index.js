...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window._showQrPopup = function showQrPopup(force) {
    if (!force && (qrShown || qrDismissed) || !qrPopup) return;
    qrShown = true;
    qrDismissed = false;
  ...
  window._hideQrPopup = function hideQrPopup() {
    if (!qrPopup) return;
  ...
  // Close popup
  if (qrClose) qrClose.addEventListener('click', window._hideQrPopup);
  if (qrOverlay) qrOverlay.addEventListener('click', window._hideQrPopup);
  ...
  // ============================================================
  // 14. STORY AUDIO PLAYER — Play button with 10-second gate
  // ============================================================
  (function initStoryAudio() {
    const REQUIRED_SECONDS = 50;

    const stories = [
      { btnId: 'playBtn1', src: 'assets/audio/Battle of the ants - 4web.mp3', title: 'Battle of the Ants' },
      { btnId: 'playBtn2', src: 'assets/audio/pareeki jathui chadi + intro.mp3', title: 'परी की जादुई छड़ी' },
      { btnId: 'playBtn3', src: 'assets/audio/mazhavillu thotta kunjan ondhu 4web.mp3', title: 'മഴവില്ല് തൊട്ട കുഞ്ഞന്‍ ഓന്ത്' },
    ];

    // Build mini-player bar
    const player = document.createElement('div');
    player.id = 'audioPlayer';
    player.innerHTML = `
      <div class="audio-player-inner">
        <svg class="audio-progress-ring" viewBox="0 0 44 44">
          <circle class="audio-ring-bg" cx="22" cy="22" r="18"/>
          <circle class="audio-ring-fill" cx="22" cy="22" r="18" id="audioRingFill"/>
        </svg>
        <button class="audio-play-pause" id="audioPlayPause" aria-label="Pause">&#10074;&#10074;</button>
        <div class="audio-info">
          <div class="audio-title" id="audioTitle"></div>
          <div class="audio-timer" id="audioTimer">0 / 50s</div>
        </div>
        <a class="audio-listen-btn" id="audioListenBtn" href="https://lalakidsstories.page.link/share" target="_blank">Listen Now</a>
        <button class="audio-close" id="audioClose" aria-label="Close">&times;</button>
      </div>
    `;
    ...
    const ringFill     = document.getElementById('audioRingFill');
    ...
    function startTimer() {
      ...
      timerInterval = setInterval(() => {
        ...
        listenedSeconds = Math.min(listenedSeconds + 1, REQUIRED_SECONDS);
        timerEl.textContent = `${listenedSeconds} / ${REQUIRED_SECONDS}s`;
        setRing(listenedSeconds / REQUIRED_SECONDS);

        if (listenedSeconds >= REQUIRED_SECONDS && !gateReached) {
          gateReached = true;
          clearInterval(timerInterval);
          audioEl.pause();
          playPauseBtn.style.display = 'none';
          timerEl.textContent = 'Preview ended';
          player.classList.add('gate-reached');
    ...
    function openPlayer(src, title) {
      ...
      if (audioEl.src !== fullSrc) {
        ...
        gateReached     = false;
        setRing(0);
        timerEl.textContent = `0 / ${REQUIRED_SECONDS}s`;
        player.classList.remove('gate-reached');
    ...
    stories.forEach(({ btnId, src, title }) => {
